...
var optionsFactory = function (options) {
  ...
  forIn(generalOptions, function (generalOption, generalOptionId) {
    // for each of lp available options
    forIn(generalOption.options, function (option) {
  ...
  forIn(oOptions, function (oOption, oOptionId) {
    // for each of lp available options
    forIn(oOption.options, function (option) {
      // for each option possible name
      if (typeof options[option] !== "undefined") {
        // check of the method options contains the value
        if (oOption.expects === "") {
          // when `expects` is '', we just mark this option as true
          if (options[option]) {
        ...
        // if method options does not contains the value, use
        // default if available
        selOOptions[oOptionId] = oOption.default;
      }
    });
    if (typeof oOption.default !== "undefined") {
      // if method options does not contains the value, use default
      // if available
